    ...
    return {
        ...
        arenaReady() {
            ...
            const translate = {
                ...
                gw_yigeniyin: '伊格尼印',
                gw_yigeniyin_info: '对敌方角色中体力值最高的一名随机角色造成1点火焰伤害,然后对场上体力值最高的所有角色各造成1点火焰伤害',
                gw_leizhoushu: '雷咒术',
                ...
                gwmaoxian_luoqi: '罗契',
                gwmaoxian_luoqi_info: '选择一名角色视为对其使用一张不计入出杀次数的【杀】,然后所有其他角色可以对目标使用一张【杀】',
                ...
                gwmaoxian_huoge: '霍格',
                gwmaoxian_huoge_info: '观看牌堆顶的六张牌,使用至多两张,然后弃掉其余的牌',
            };
        ...
        precontent() {
            ...
            const skill = {
                ...
                // 每有一个质疑者,你增加一次蛊惑次数
                // 拥有<惑>标记的角色不可质疑你的蛊惑
                // 每回合结束时你可以移除惑标记,然后视为使用移除的标记数张任意牌
                蛊惑: {
                    mod: {
                                ...
                                .set('ai', function (event, player) {
                                    function calculateMaxEffect(choice) {
                                        ...
                                        for (const npc of game.players) {
                                            ...
                                            if (effectValue > maxEffect) maxEffect = effectValue;
                                        }
                                        return maxEffect;
                                    }
                    ...
                    async content(event, trigger, player) {
                        const {
                            ...
                            if (trigger.player == player) {
                                ...
                                if (player.isPhaseUsing() && player.hasValueTarget(card, null, true)) return -1;
                                return 10;
                            } else {
                                if (player.isPhaseUsing() && player.hasValueTarget(card, null, true)) return -1;
                ...
                会玩: {
                    ...
                    async content(event, trigger, player) {
                        ...
                        const {
                            result: { links },
                        } = await player.chooseButton(['请选择卡牌', list], true, trigger.num).set('ai', (button) => get.value(button.link));
                        if (links && links[0]) {
                ...
                //——————————————————————————————————————————————————————————————————————————————————————————————————孙尚香
                //結姻
                // 回合每名角色限一次,你可以选择一名角色,然后你弃置其一张手牌或将其一张装备牌置入你的装备区.若如此做,你摸一张牌并回复1点体力
                QD_jieyin: {
                    enable: 'phaseUse',
                    filterTarget(card, player, target) {
                ...
                //——————————————————————————————————————————————————————————————————————————————————————————————————大乔
                //國色
                // 回合每名角色限一次,你可以观看并弃置其区域内的一张◆牌,然后你选择一项:1.视为对一名角色使用一张【乐不思蜀】;2.移动或弃置场上一张【乐不思蜀】.若如此做,你摸一张牌
                QD_guose: {
                    enable: 'phaseUse',
                    filterTarget(card, player, target) {
                ...
                //——————————————————————————————————————————————————————————————————————————————————————————————————孙权
                //制衡
                // 回合限一次,你可以弃置一名角色任意张牌,然后摸等量的牌(若弃置了一个区域内的所有牌,则多摸一张牌)
                QD_zhiheng: {
                    enable: 'phaseUse',
                    filterTarget(card, player, target) {
                        return target.countCards('he');
                ...
                // 救援
                // 当其他角色使用【桃】时,你可以令此牌目标改为你,然后你摸一张牌.其他角色对你使用的【桃】回复的体力值+1.当你需要使用【桃】时,你可以令任意其他角色代替你使用一张【桃】,否则该角色失去一点体力
                QD_jiuyuan: {
                    trigger: {
                        global: ['taoBegin'],
                    },
                ...
                //——————————————————————————————————————————————————————————————————————————————————————————————————周泰
                //不屈
                // 当你进入濒死状态时,你可以令一名角色展示牌堆顶一张牌,若此牌与其武将牌上的不屈牌点数均不同,你将此牌置于其武将牌上,然后将体力回复至1.否则你获得所有不屈牌,然后其执行一次濒死结算,若其因此死亡,则终止你的濒死结算.你的手牌上限+全场不屈牌的数量
                QD_buqu: {
                    trigger: {
                        player: ['dyingBefore'],
                    },
                ...
                QD_gongxin: {
                    ...
                    forced: true,
                    filter(event, player) {
                        if (_status.jieduan.QD_gongxin) return false;
                        if (event.player == player) {
                ...
                //——————————————————————————————————————————————————————————————————————————————————————————————————兀突骨
                // 燃殇
                // 当你受到其他角色伤害后,伤害来源获得伤害数个<燃殇>.任意角色失去牌时,有<燃殇>的角色失去<燃殇>数点体力
                QD_ranshang: {
                    intro: {
                        content: 'mark',
                    },
                    ...
                    async content(event, trigger, player) {
                        trigger.source.addMark('QD_ranshang', numberq1(trigger.num));
                    },
                    group: ['QD_ranshang_1'],
                    subSkill: {
                        1: {
                            ...
                            forced: true,
                            filter(event, player) {
                                return game.players.some((q) => q.storage.QD_ranshang > 0);
                            },
                            async content(event, trigger, player) {
                                for (const npc of game.players.filter((q) => q.storage.QD_ranshang > 0)) {
                                    npc.loseHp();
                                }
                            },
                        },
            ...
            const translate = {
                ...
                QD_zhangrang: '张让',
                QD_taoluan: '滔乱',
                QD_taoluan_info: '每种牌名每阶段限一次,你可以将一张牌当任意牌使用或打出,然后你选择一名其他角色令其选择①交给你一张牌②失去一点体力并随机失效一个技能',
                //——————————————————————————————————————————————————————————————————————————————————————————————————荀攸
                QD_xunyou: '荀攸',
                QD_qice: '奇策',
                ...
                QD_caozhi: '曹植',
                QD_luoying: '落英',
                QD_luoying_info: '任意角色不因<重铸/使用>而失去♣️️牌时,你获得之然后翻回正面.你的出牌阶段外,删除此技能括号内内容',
                QD_jiushi: '酒诗',
                ...
                QD_神司马: '神司马',
                忍戒: '忍戒',
                忍戒_info: '当你改变体力值或出牌阶段外失去牌时,你获得等量的忍.当你的忍大于3,你获得<极略>,然后增加一点体力上限,回复两点体力,摸两张牌',
                极略: '极略',
                ...
                QD_严颜: '严颜',
                拒战: '拒战',
                拒战_info: '当你成为其他角色牌的目标后,你与其各摸一张牌,然后其本回合内不能再对你使用牌.当你使用牌指定任意角色为目标后,你获得其一张牌',
                //——————————————————————————————————————————————————————————————————————————————————————————————————————司马师
                QD_司马师: '司马师',
                夷灭: '夷灭',
                ...
                QD_weimu: '帷幕',
                QD_weimu_info: '由你自己作为来源或回合内体力值与体力上限变化后,你摸变化值两倍的牌,然后免疫之.你不能成为黑色牌的目标',
                QD_wansha: '完杀',
                ...
                橘: '橘',
                橘_info: '游戏开始时,你获得6个<橘>;有<橘>的角色摸牌阶段多摸2张牌;摸牌阶段开始前,你获得2个<橘>.当有<橘>的角色受到伤害时,防止此伤害,然后其移去1个<橘>',
                //——————————————————————————————————————————————————————————————————————————————————————————————————————张角
                QD_张角: '张角',
                QD_leiji: '雷击',
                ...
                QD_袁术: '袁术',
                QD_wangzun: '妄尊',
                QD_wangzun_info: '其他角色准备阶段你可以摸一张牌,然后其本回合只能对你使用牌,且手牌上限减三',
                QD_wangzun_1: '被妄尊',
                ...
                QD_夏侯渊: '夏侯渊',
                奇兵: '奇兵',
                奇兵_info: '任意角色的结束阶段结束时,你摸一张牌,然后可以使用一张牌,此牌无距离限制',
                夺锐属性: '夺锐属性',
                ...
                埋祸: '埋祸',
                埋祸_info: '其他角色对你使用牌时,你可以将此牌置于其武将牌上称为<祸>并令其失效<br>当你对其他角色使用牌时,移去其武将牌上的一张<祸><br>其他角色出牌阶段开始时,随机失去一半的<祸>(向上取整),然后对你使用剩余的<祸>',
                //——————————————————————————————————————————————————————————————————————————————————————————————————————宣公主
                QD_宣公主: '宣公主',
                比翼: '比翼',
                ...
                QD_dinghuo: '绽火',
                QD_dinghuo_info: '限定技,出牌阶段,你可以移去全部<军略>标记,令至多等量的已横置角色弃置所有装备区内的牌.然后,你对其中一名其他角色造成军略数点火焰伤害.',
                //——————————————————————————————————————————————————————————————————————————————————————————————————————春哥
                QD_chunge: '春哥',
                QD_jueqing: '绝情',
                ...
                QD_dengai: '邓艾',
                QD_tuntian: '屯田',
                QD_tuntian_info: '<出牌阶段外失去牌/出牌阶段内不因使用而失去牌>后,你可以获得其他角色的y张牌(y不大于2x),然后摸2x-y张牌(x为你失去牌的数量)',
                //——————————————————————————————————————————————————————————————————————————————————————————————————陆逊
                QD_luxun: '陆逊',
                QD_qianxun: '谦逊',
                ...
                QD_zhoutai: '周泰',
                QD_buqu: '不屈',
                QD_buqu_info: '当你进入濒死状态时,你可以令一名角色展示牌堆顶一张牌,若此牌与其武将牌上的不屈牌点数均不同,你将此牌置于其武将牌上,然后将体力回复至1.否则你获得所有不屈牌,然后其执行一次濒死结算.若其因此死亡,则终止你的濒死结算.你的手牌上限+全场不屈牌的数量',
                //——————————————————————————————————————————————————————————————————————————————————————————————————陆抗
                QD_lukang: '陆抗',
                QD_qianjie: '谦节',
                ...
                QD_zhouyu: '周瑜',
                QD_yingzi: '英姿',
                QD_yingzi_info: '你不因此技能获得牌时摸一张牌,每轮开始时,你可以令一名其他角色于本轮获得牌时随机少获得一张牌',
                QD_yingzi_2: '英姿',
                QD_yingzi_2_info: '获得牌时随机少获得一张牌',
                QD_fanjian: '反间',
                QD_fanjian_info: '回合每种花色限一次,你可以声明一个花色然后获得一名其他角色一张牌.若此牌花色与你声明的花色不同,其弃置与此牌花色相同的牌.若其因此弃置了牌,其失去1点体力',
                //——————————————————————————————————————————————————————————————————————————————————————————————————华佗
                QD_huatuo: '华佗',
                QD_jijiu: '急救',
                ...
                QD_daqiao: '大乔',
                QD_guose: '國色',
                QD_guose_info: '回合每名角色限一次,你可以观看并弃置其区域内的一张◆牌,然后你选择一项:1.视为对一名其他角色使用一张【乐不思蜀】;2.移动或弃置场上一张【乐不思蜀】.若如此做,你摸一张牌',
                QD_liuli: '流離',
                ...
                QD_jieyin: '結姻',
                QD_jieyin_info: '回合每名角色限一次,你可以选择一名其他角色,然后你弃置其一张手牌或将其一张装备牌置入你的装备区.若如此做,你摸一张牌并回复1点体力',
                QD_xiaoji: '枭姬',
